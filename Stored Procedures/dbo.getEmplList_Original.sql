...
-- =============================================
CREATE PROCEDURE [dbo].[getEmplList_Original]
...
BEGIN
	...
	SELECT distinct
		...
		,(select top 1
				...
				and ase.isActive = 1
				and ase.isDeleted = 0
				and ((ase.IsPrimary = 1 and @UserRoleID = 1) OR (@UserRoleID = 2 and s.EmplID = @ncUserId))) SubEvalID  --if its manager , then get the primary subeval or if its subeval, then get the matching subeval id.
		,e.NameFirst
		,e.NameMiddle
		,e.NameLast
