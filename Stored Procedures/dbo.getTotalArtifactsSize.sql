...
-- Description:	Get total file size for artifacts
-- =============================================
CREATE PROCEDURE [dbo].[getTotalArtifactsSize]
@InActivePlan as bit 
AS
BEGIN
	SET NOCOUNT ON;
	if @InActivePlan = 0
	BEGIN
		select SUM(FileSize) as TotalSize  from Evidence where IsDeleted = 0
	END
	ELSE
	BEGIN
