...
AS
    BEGIN
        ...
        SET @CurrentSchYear = ( SELECT DISTINCT
                                        RTRIM(SchYear)
...
--print @tmp
        ...
        SELECT  rptedt.PlanEndDateID ,
                ...
                ( CASE WHEN cdEdt.CodeText = 'End of Year One'
                       THEN ( CASE WHEN CONVERT(INT, SUBSTRING(rptedt.DefaultPlanEndDate,
                                   ...
                                   THEN RTRIM(rptedt.DefaultPlanEndDate) + '/'
                                        + SUBSTRING(@CurrentSchYear, 6, 4) -- '1-6' 
                                   ELSE RTRIM(rptedt.DefaultPlanEndDate) + '/'
                                        + SUBSTRING(@CurrentSchYear, 1, 4) -- '7-12'
                              END )
                       ...
                       THEN ( CASE WHEN CONVERT(INT, SUBSTRING(rptedt.DefaultPlanEndDate,
                                                              ...
                                                              - 1)) < 7
                                   THEN RTRIM(rptedt.DefaultPlanEndDate) + '/'
                                        + SUBSTRING(@NextSchYear, 6, 4) -- '1-6' 
                                   ELSE RTRIM(rptedt.DefaultPlanEndDate) + '/'
                                        + SUBSTRING(@NextSchYear, 1, 4) -- '7-12'
                              END )
			--When cdEdt.CodeText = 'Duration Greater than' Then Convert(Varchar,1+ DATEADD(dd,CONVERT(int, rptedt.DefaultPlanEndDate),getdate() ),101 )			
                ...
                ( CASE WHEN ( rptedt.DefaultFormativeValue != ''
                       ...
                       THEN ( CASE WHEN CONVERT(INT, SUBSTRING(rptedt.DefaultPlanEndDate,
                                   ...
                                   THEN RTRIM(rptedt.DefaultFormativeValue)
                                        + '/' + SUBSTRING(@CurrentSchYear, 6,
                                                          4) -- '1-6' 
                                   ELSE RTRIM(rptedt.DefaultFormativeValue)
                                        + '/' + SUBSTRING(@CurrentSchYear, 1,
                                                          4) -- '7-12'
                              END )
                ...
                ( CASE WHEN cdEdt.CodeText = 'Duration Greater than'
                       ...
                       THEN CONVERT(VARCHAR(10), ISNULL(( SELECT TOP 1
                                                              CalendarDate
                                                          FROM
                                                              dbo.SchoolCalendar
                                                          WHERE
        ...
        FROM    dbo.RubricPlanType rpt
                INNER JOIN dbo.RubricPlanTypeEndDate rptedt ON rpt.RubricPlanTypeID = rptedt.RubricPlanTypeID
                INNER JOIN dbo.CodeLookUp cdEdt ON rptedt.PlanEndDateTypeID = cdEdt.CodeID
                                               AND cdEdt.CodeType = 'EndDtType'
                                               AND cdEdt.CodeActive = 1
                INNER JOIN dbo.CodeLookUp cdE ON rptedt.EndTypeID = cdE.CodeID
                                             AND cdE.CodeType = 'EndType'
                                             AND cdE.CodeActive = 1
                INNER JOIN dbo.RubricHdr rh ON rh.RubricID = rpt.RubricID
                LEFT JOIN dbo.CodeLookUp clRptPl ON clRptPl.CodeID = rpt.PlanTypeID
